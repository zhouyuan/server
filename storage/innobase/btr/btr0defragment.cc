...
start. */
simple_atomic_counter<> btr_defragment_compression_failures;
/* Number of btr_defragment_n_pages calls that altered page but didn't
manage to release any page. */
simple_atomic_counter<> btr_defragment_failures;
...
simple_atomic_counter<> btr_defragment_count;
	...
	mtr_t*		mtr)		/*!< in/out: mini-transaction */
{
	...
	while (n_recs_to_move > 0) {
		...
		btr_defragment_compression_failures.inc();
		max_ins_size_to_use =
	...
	mtr_t*		mtr)	/*!< in/out: mini-transaction */
{
	...
	mem_heap_free(heap);
	n_defragmented ++;
	btr_defragment_count.inc();
	if (n_pages == n_defragmented) {
		btr_defragment_failures.inc();
	} else {
