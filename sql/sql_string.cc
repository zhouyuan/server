...
/**
   ...
   before copying and the old buffer freed.
...
bool String::append(const char *s,size_t size)
{
  ...
  */
  if (charset()->mbminlen > 1)
  {
    uint32 add_length=arg_length * charset()->mbmaxlen;
    uint dummy_errors;
  ...
  */
  return Binary_string_with_extra_alloc::append(s, size);
}


bool Binary_string_with_extra_alloc::append(const char *s, size_t size)
{
  DBUG_ASSERT(size <= UINT_MAX32);
  uint32 arg_length= (uint32) size;
...
bool String::append(const char *s, size_t arg_length, CHARSET_INFO *cs)
{
  ...
  if (needs_conversion((uint32)arg_length, cs, charset(), &offset))
  {
    ...
    {
      DBUG_ASSERT(charset()->mbminlen > offset);
      offset= charset()->mbminlen - offset; // How many characters to pad
    ...
    add_length= arg_length / cs->mbminlen * charset()->mbmaxlen;
    uint dummy_errors;
    if (realloc_with_extra_if_needed(str_length + add_length)) 
      return TRUE;
...
bool Binary_string_with_extra_alloc::append(IO_CACHE* file, uint32 arg_length)
{
  if (realloc_with_extra_if_needed(str_length+arg_length))
    return TRUE;
...
*/
bool Binary_string_with_extra_alloc::append_parenthesized(long nr, int radix)
{
  char buff[64], *end;
  buff[0]= '(';
...
bool Binary_string_with_extra_alloc::append_with_prefill(const char *s,
                                                         uint32 arg_length,
                                                         uint32 full_length,
                                                         char fill_char)
{
...
{
  return (uint32) charset()->cset->numchars(charset(), Ptr, Ptr+str_length);
}
...
int String::charpos(longlong i,uint32 offset)
{
  if (i <= 0)
    return (int)i;
  return (int) charset()->cset->charpos(charset(),Ptr+offset,Ptr+str_length,(size_t)i);
}

int Static_string::strstr(const Static_string &s, uint32 offset)
{
...
int Static_string::strrstr(const Static_string &s, uint32 offset)
{
...
bool Binary_string_with_extra_alloc::replace(uint32 offset,uint32 arg_length,
...
bool Binary_string_with_extra_alloc::append_for_single_quote(const char *st,
                                                             size_t len)
{
