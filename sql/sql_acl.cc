...
bool mysql_create_user(THD *thd, List <LEX_USER> &list, bool handle_as_role)
{
  ...
  bool create_or_replace= thd->lex->create_info.or_replace();
  bool multiple_users= list.elements - 1;
...
log:
    if (mysql_bin_log.is_open() && !handle_as_role && multiple_users)
    {
      if (create_or_replace)
        binlog_query.append("CREATE OR REPLACE USER ");
      else
